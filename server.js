...
// -------------------------------------------------------------------------
//  ★ 宽松艺术家比较（兼容 lx‑music‑desktop） ★
// -------------------------------------------------------------------------
// lx‑music‑desktop 在搜索时会把艺术家字符串拆分为数组（/,、&、;、|、\\ 等分隔符），
// 并对每一项做统一的大小写、空格、标点、特殊字符去除后再比较。
// Mineradio 原实现使用 `lxStyleNormalizeText` 对整串艺术家做一次性归一化，
// 然后直接做 **完全相等** 判断，导致 "Justin Bieber / Ludacris" 与 "Justin Bieber，Ludacris"
// 不匹配。
// 为了兼容两套实现，我们在此提供两段工具函数：
//   1️⃣ `normalizeArtistList(str)` – 把艺术家字符串拆分为 “归一化数组”
//   2️⃣ `isArtistMatch(a, b)`      – 只要两侧数组有交集即认为匹配。
// 这两个函数的行为与 lx‑music‑desktop 中 `formatSingerName` 基本一致，
// 能容忍 “/、,、&、;、空格、下划线、点号”等分隔符以及大小写差异。
// 之后在 `searchCrossPlatformCandidate`（跨平台查询核心）中把
//   `fSinger === targetSinger`
// 替换为
//   `isArtistMatch(fSinger, targetSinger)`
// 这样即使各平台返回的艺术家顺序、分隔符不同，只要包含同一位艺术家就能匹配成功，
// LX 源的 `musicUrl` 解析步骤将被顺利调用。
// -------------------------------------------------------------------------
function normalizeArtistList(str) {
  ...
  return replaced
    ...
    .map(s => s
      .trim()
      .replace(/[\s\-_\.]+/g, '')   // 删除空格、下划线、破折号、点号
      .replace(/[^\w一-龥]/g, ''))  // 修复：保留中文字符（一-龥）
    .filter(Boolean);
}
...
// -------------------------------------------------------------------------
//  统一抽取艺术家（对所有平台的搜索结果做一次标准化）
// -------------------------------------------------------------------------
function extractArtist(candidate) {
  if (!candidate) return '';
  if (typeof candidate.artist === 'string' && candidate.artist.trim()) {
    return candidate.artist.trim();
  }
  if (Array.isArray(candidate.artists) && candidate.artists.length) {
    const names = candidate.artists
      .map(a => (a && (a.name || a.artist || a.title || a.nick)))
      .filter(Boolean);
    return names.join(' / ');
  }
  if (typeof candidate.singer === 'string' && candidate.singer.trim()) {
    return candidate.singer.trim();
  }
  if (typeof candidate.author === 'string' && candidate.author.trim()) return candidate.author.trim();
  if (typeof candidate.performer === 'string' && candidate.performer.trim()) return candidate.performer.trim();
  return '';
}
...
const {
  ...
  sanitizeHeaders: sanitizeLxProxyHeaders,
} = require('./lx-source-runtime');
const {
...
const LX_SOURCE_STORE_FILE = process.env.MINERADIO_LX_SOURCE_FILE || path.join(__dirname, 'data', 'lx-sources.json');
const LX_AUDIO_PROXY_CONTEXT_TTL_MS = 10 * 60 * 1000;
const lxSourceManager = createLxSourceManager({ storeFile: LX_SOURCE_STORE_FILE });
const lxAudioProxyContexts = new Map();

// 业务平台 (netease/qq/kugou/qishui/spotify) 映射到 LX 源内置的来源代码 (wy/tx/kg/mg)
function mapSongProviderToLxSource(provider) {
  const key = String(provider || '').trim().toLowerCase();
  if (key === 'netease') return 'wy';
  if (key === 'qq' || key === 'qishui') return 'tx';
  if (key === 'kugou') return 'kg';
  if (key === 'spotify') return 'mg';
  return '';
}

// 把 LX desktop 的"匹配算法"移植到本项目：跨平台搜索结果用 duration + title + singer + album 排序挑最佳匹配
...
function lxStyleSplitArtists(artist) {
  return String(artist || '').split(/\s*\/\s*|\s*,\s*|、|&| feat\.? | ft\.? /i)
    .map(lxStyleNormalizeText).filter(Boolean);
}
function lxStyleParseInterval(value) {
  if (!value) return 0;
...
function lxStyleFindBestMatch(target, list) {
  ...
  // 第二轮：singer 匹配 + name 包含（关键修复：使用宽松匹配）
  for (const c of passInterval) {
  ...
  // 第四轮：放宽name匹配，但歌手必须匹配（避免匹配到翻唱版）
  for (const c of passInterval) {
    if (isArtistMatch(targetArtistRaw, c.item.artist) && (c.fMusicName.includes(fMusicName) || fMusicName.includes(c.fMusicName))) return c.item;
  }
  return null;
}

// 按目标平台调用对应 SDK 搜索歌曲；返回命中的搜索项（携带平台 ID）
async function searchCrossPlatformCandidate(targetPlatform, songInput) {
  const query = String(songInput && (songInput.query || '') || '').trim()
    || [songInput && songInput.name, songInput && songInput.artist].filter(Boolean).join(' ').trim();
  if (!query) return null;
  const limit = 12;
  const interval = songInput.durationMs && songInput.durationMs > 1000
    ? Math.round(songInput.durationMs / 1000)
    : 0;
  const target = {
    name: songInput.name || '',
    artist: songInput.artist || '',
    album: songInput.album || '',
    interval: lxStyleFormatInterval(interval),
    durationMs: songInput.durationMs || 0,
  };
  let list = [];
  try {
    if (targetPlatform === 'wy') {
      list = await handleSearch(query, limit, 0) || [];
    } else if (targetPlatform === 'tx') {
      list = await handleQQSearch(query, limit, 0) || [];
    } else if (targetPlatform === 'kg') {
      list = await handleKugouSearch(query, limit, kugouCookie, 0) || [];
    }
  } catch (err) {
    console.warn('[LXFallback] cross-platform search failed:', targetPlatform, err && (err.message || err));
    return null;
  }
  if (!list.length) return null;
  return lxStyleFindBestMatch(target, list);
}
function lxStyleFormatInterval(seconds) {
...
// 把搜索结果转成 LX script 的 musicInfo 结构
function buildLxMusicInfoFromMatch(sourceKey, match) {
  ...
  if (sourceKey === 'wy') {
    ...
    if (match.duration || match.dt) meta.interval = lxStyleFormatInterval(Math.round((Number(match.duration) || Number(match.dt) || 0) / 1000));
  } else if (sourceKey === 'tx') {
    songId = String(match.mid || match.songmid || match.id || '');
    meta.songId = songId;
    meta.strMediaMid = String(match.mediaMid || (match.file && match.file.media_mid) || '');
    if (match.albumMid) meta.albumMid = String(match.albumMid);
    if (match.album || match.albumName) meta.albumName = String(match.album || match.albumName);
    if (match.cover || match.picUrl) meta.picUrl = String(match.cover || match.picUrl);
  } else if (sourceKey === 'kg') {
    songId = String(match.hash || match.fileHash || '');
    meta.songId = songId;
    meta.hash = songId;
    if (match.albumId) meta.albumId = String(match.albumId);
    if (match.album || match.albumName) meta.albumName = String(match.album || match.albumName);
  ...
  // LX 脚本（如 Huibq 等）直接读 musicInfo.hash / musicInfo.songmid 获取歌曲ID，
  // 所以必须把平台 ID 同时放在顶层和 meta 中
  return {
    name: match.name || '',
    singer: Array.isArray(match.artists) && match.artists.length
      ? match.artists.map((a) => (a && a.name) || '').filter(Boolean)
      : lxStyleSplitArtists(match.artist),
    source: sourceKey,
    interval: intervalSeconds,  // 改进：直接返回秒数，而不是格式化字符串
    // 顶层 ID 字段（LX 脚本读取位置）
    hash: sourceKey === 'kg' ? songId : undefined,
    songmid: sourceKey === 'tx' || sourceKey === 'wy' ? songId : undefined,
    strMediaMid: sourceKey === 'tx' ? (meta.strMediaMid || undefined) : undefined,
    copyrightId: sourceKey === 'mg' ? songId : undefined,
    meta: Object.assign({}, meta, {
      // 改进：添加 qualitys 字段，让LX音源知道可以尝试哪些音质
...
function rememberLxAudioProxyContext(headers) {
  const cleaned = sanitizeLxProxyHeaders(headers);
  const keys = Object.keys(cleaned);
  if (!keys.length) return '';
  const token = crypto.randomBytes(18).toString('hex');
  lxAudioProxyContexts.set(token, {
    headers: cleaned,
    expiresAt: Date.now() + LX_AUDIO_PROXY_CONTEXT_TTL_MS,
  });
  return token;
}
function pruneLxAudioProxyContexts() {
  const now = Date.now();
  for (const [token, entry] of lxAudioProxyContexts.entries()) {
    if (!entry || entry.expiresAt <= now) lxAudioProxyContexts.delete(token);
  }
}
function lxAudioProxyContext(token) {
  pruneLxAudioProxyContexts();
  const key = String(token || '').trim().toLowerCase();
  if (!/^[a-f0-9]{36}$/i.test(key)) return null;
  const entry = lxAudioProxyContexts.get(key);
  if (!entry || entry.expiresAt <= Date.now()) {
    lxAudioProxyContexts.delete(key);
    return null;
  }
  entry.expiresAt = Date.now() + LX_AUDIO_PROXY_CONTEXT_TTL_MS;
...
function audioProxyHeadersFor(audioUrl, range, extraHeaders) {
  ...
  const safeExtra = sanitizeLxProxyHeaders(extraHeaders);
  Object.keys(safeExtra).forEach((key) => {
    if (key === 'range') return;
    headers[key === 'user-agent' ? 'User-Agent' : (key === 'referer' ? 'Referer' : (key === 'origin' ? 'Origin' : (key === 'cookie' ? 'Cookie' : key)))] = safeExtra[key];
  });
...
async function handleQQSongUrl(mid, mediaMid, qualityPreference, playbackHints) {
  ...
  // 修复：如果QQ音乐无法播放且启用了LX音源，强制触发LX接管
  const lxSources = lxSourceManager && lxSourceManager.listSources ? lxSourceManager.listSources() : [];
  const hasReadyLxSource = lxSources.some(s => s.enabled && s.initStatus === 'ready');

  if (hasReadyLxSource && restriction.category !== 'playback_key_missing') {
    console.log('[QQSongUrl] 无法播放，已启用LX音源，返回 trial_only 触发LX接管');
    return {
      provider: 'qq',
      url: '',
      playable: false,
      ...
      userId: hasQQPlaybackSession ? uin : '',
      vipRequired: memberTrackHint,
      restriction: Object.assign({}, restriction, { category: 'trial_only', reason: 'trial_only' }),
      reason: 'trial_only',
      category: 'trial_only',
      message: 'QQ音乐无法播放，尝试使用LX音源',
      requestedQuality,
    };
  }
...
async function resolveNeteaseDirectSongUrl(id, loginInfo, qualityPreference) {
  ...
  // 修复：如果有试听片段但启用了LX音源，返回空URL强制触发LX接管
  if (trialFallback) {
    console.log('[SongUrl] 检测到试听片段 trialFallback:', {
      url: trialFallback.url ? '有URL' : '无URL',
      trial: trialFallback.trial,
      level: trialFallback.level
    });
    ...
    console.log('[SongUrl] LX音源状态:', {
      总数: lxSources.length,
      已就绪: lxSources.filter(s => s.enabled && s.initStatus === 'ready').length,
      hasReadyLxSource
    });

    if (hasReadyLxSource) {
      console.log('[SongUrl] ✓ 已启用LX音源，返回空URL以触发LX接管');
      const restriction = classifyNeteasePlaybackRestriction(lastData, loginInfo);
      return {
        provider: 'netease',
        source: 'netease',
        url: null,  // 关键：不返回试听URL
        trial: false,
        playable: false,
        reason: 'trial_only',
        category: 'trial_only',
        message: '检测到试听片段，尝试使用LX音源获取完整版',
        restriction: Object.assign({}, restriction, { category: 'trial_only', reason: 'trial_only' }),
        _hadTrialUrl: true,  // 标记原本有试听URL
        _originalTrialInfo: trialFallback.trialInfo,
        requestedQuality,
      };
    }
    console.log('[SongUrl] ✗ LX音源不可用，返回试听片段');
    // LX音源不可用，返回试听片段
...
function lxSongLabel(song) {
  return [song && song.lxSourceName, song && song.lxOriginSource].filter(Boolean).join(' · ');
}

async function importLxSourceFromUrl(sourceUrl) {
  const targetUrl = String(sourceUrl || '').trim();
  if (!/^https?:\/\//i.test(targetUrl)) throw new Error('LX_SOURCE_URL_INVALID');
  const response = await fetchWithTimeout(targetUrl, {
    headers: { 'User-Agent': `Mineradio/${APP_VERSION}` },
  }, 8000);
  if (!response.ok) throw new Error('LX_SOURCE_URL_HTTP_' + response.status);
  const rawScript = await response.text();
  return lxSourceManager.importSource({ sourceType: 'url', sourceUrl: targetUrl, rawScript });
}
...
const server = http.createServer(async (req, res) => {
  ...
  if (pn === '/api/lx/sources') {
    try {
      sendJSON(res, { ok: true, sources: lxSourceManager.listSources() });
    } catch (err) {
      console.error('[LXSourceList]', err);
      sendJSON(res, { ok: false, sources: [], error: err.message || 'LX_SOURCE_LIST_FAILED' }, 500);
    }
    return;
  }

  if (pn === '/api/lx/source/import-file') {
    try {
      if (req.method !== 'POST') { sendJSON(res, { ok: false, error: 'METHOD_NOT_ALLOWED' }, 405); return; }
      const body = await readRequestBody(req);
      const imported = await lxSourceManager.importSource({
        sourceType: 'file',
        sourcePath: body.sourcePath || body.filePath || '',
        rawScript: body.rawScript || body.text || '',
      });
      sendJSON(res, { ok: true, source: imported, sources: lxSourceManager.listSources() });
    } catch (err) {
      console.error('[LXSourceImportFile]', err);
      sendJSON(res, { ok: false, error: err.code || err.message || 'LX_SOURCE_IMPORT_FAILED' }, 400);
    }
    return;
  }
  ...
  if (pn === '/api/lx/source/import-url') {
    try {
      if (req.method !== 'POST') { sendJSON(res, { ok: false, error: 'METHOD_NOT_ALLOWED' }, 405); return; }
      const body = await readRequestBody(req);
      const imported = await importLxSourceFromUrl(body.url || body.sourceUrl || '');
      sendJSON(res, { ok: true, source: imported, sources: lxSourceManager.listSources() });
    } catch (err) {
      console.error('[LXSourceImportUrl]', err);
      sendJSON(res, { ok: false, error: err.code || err.message || 'LX_SOURCE_IMPORT_URL_FAILED' }, 400);
    }
    return;
  }

  if (pn === '/api/lx/source/update') {
    try {
      if (req.method !== 'POST') { sendJSON(res, { ok: false, error: 'METHOD_NOT_ALLOWED' }, 405); return; }
      const body = await readRequestBody(req);
      const updated = await lxSourceManager.updateSource(String(body.id || ''), {
        enabled: body.enabled,
        order: body.order,
      });
      sendJSON(res, { ok: true, source: updated, sources: lxSourceManager.listSources() });
    } catch (err) {
      console.error('[LXSourceUpdate]', err);
      sendJSON(res, { ok: false, error: err.code || err.message || 'LX_SOURCE_UPDATE_FAILED' }, 400);
    }
    return;
  }
  ...
  if (pn === '/api/lx/source/delete') {
    try {
      if (req.method !== 'POST') { sendJSON(res, { ok: false, error: 'METHOD_NOT_ALLOWED' }, 405); return; }
      const body = await readRequestBody(req);
      const removed = await lxSourceManager.deleteSource(String(body.id || ''));
      sendJSON(res, { ok: true, source: removed, sources: lxSourceManager.listSources() });
    } catch (err) {
      console.error('[LXSourceDelete]', err);
      sendJSON(res, { ok: false, error: err.code || err.message || 'LX_SOURCE_DELETE_FAILED' }, 400);
    }
    return;
  }
  ...
  if (pn === '/api/lx/source/reload') {
    try {
      if (req.method !== 'POST') { sendJSON(res, { ok: false, error: 'METHOD_NOT_ALLOWED' }, 405); return; }
      const body = await readRequestBody(req);
      const updated = await lxSourceManager.reloadSource(String(body.id || ''));
      sendJSON(res, { ok: true, source: updated, sources: lxSourceManager.listSources() });
    } catch (err) {
      console.error('[LXSourceReload]', err);
      sendJSON(res, { ok: false, error: err.code || err.message || 'LX_SOURCE_RELOAD_FAILED' }, 400);
    }
    return;
  }
  ...
  if (pn === '/api/lx/search') {
    try {
      ...
      const result = sourceId
        ? await lxSourceManager.searchSource(lxSourceManager.findRecord(sourceId), kw, limit, offset)
        : await lxSourceManager.searchAll(kw, limit, offset);
      sendJSON(res, Object.assign({ provider: 'lx' }, result));
    } catch (err) {
      console.error('[LXSearch]', err);
      sendJSON(res, { provider: 'lx', songs: [], error: err.code || err.message || 'LX_SEARCH_FAILED' }, 500);
    }
    return;
  }

  if (pn === '/api/lx/song/url') {
    console.log('[LXSongUrl] ========== 接口被调用 ==========');
    console.log('[LXSongUrl] 请求时间:', new Date().toISOString());
    try {
      if (req.method !== 'POST') { sendJSON(res, { ok: false, error: 'METHOD_NOT_ALLOWED' }, 405); return; }
      const body = await readRequestBody(req);
      console.log('[LXSongUrl] 请求body:', JSON.stringify(body, null, 2));
      const startTime = Date.now();
      const result = await lxSourceManager.resolveSongUrl(body.song || body);
      const elapsed = Date.now() - startTime;
      console.log('[LXSongUrl] resolveSongUrl返回，耗时:', elapsed, 'ms');
      console.log('[LXSongUrl] 返回结果:', { hasUrl: !!result.url, error: result.error, url: result.url?.substring(0, 100) });
      const proxyContextToken = rememberLxAudioProxyContext(result.headers);
      sendJSON(res, Object.assign({}, result, {
        playable: !!result.url,
        proxyContextToken,
      }));
    } catch (err) {
      console.error('[LXSongUrl] 发生错误:', err);
      sendJSON(res, { provider: 'lx', url: '', playable: false, error: err.code || err.message || 'LX_SONG_URL_FAILED' }, 500);
    }
    return;
  }

  // LX 源接管：先按原平台来源用应用自身的搜索 SDK（netease/qq/kugou）找到同名同艺人候选，
  // 然后用候选的 ID 调用 LX 脚本的 musicUrl 拿到可播放 URL。
  // 这样即使 LX 脚本只暴露 musicUrl 也能完成接管。
  // body: { source: 'netease'|'qq'|'kugou'|'qishui'|'spotify', song: { name, artist, artists, duration, album, ... }, quality? }
  if (pn === '/api/lx/search-and-url') {
    console.log('[LXSearchAndUrl] ========== 接口被调用 ==========');
    try {
      if (req.method !== 'POST') { sendJSON(res, { ok: false, error: 'METHOD_NOT_ALLOWED' }, 405); return; }
      const body = await readRequestBody(req);
      console.log('[LXSearchAndUrl] 请求参数:', {
        source: body.source,
        songName: body.song?.name,
        songArtist: body.song?.artist,
        quality: body.quality
      });
      const sourceKey = mapSongProviderToLxSource(body && body.source);
      const songInput = (body && body.song) || {};
      const artists = Array.isArray(songInput.artists) && songInput.artists.length
        ? songInput.artists.map((a) => a && a.name).filter(Boolean)
        : [];
      const artistName = artists[0] || songInput.artist || songInput.artistName || '';
      const query = [songInput.name || songInput.title || '', artistName].filter(Boolean).join(' ').trim();
      if (!query || !sourceKey) {
        sendJSON(res, { provider: 'lx', url: '', playable: false, error: 'LX_SEARCH_INPUT_INVALID' });
        return;
      }
      const quality = String(body.quality || '').trim();
      const songInputWithQuery = Object.assign({}, songInput, {
        artist: artistName,
        artists,
        query,
        durationMs: Number(songInput.duration || songInput.durationMs || songInput.dt || 0) || 0,
      });
      // 1) 在原平台以外的其它平台找匹配的同首歌（避免 VIP 同平台受限）
      // 优先顺序：QQ音乐 > 酷狗 > 网易云（因为周杰伦等歌手在网易云搜索困难）
      const probeSourceOrder = sourceKey === 'wy'
        ? ['tx', 'kg', 'mg']
        : sourceKey === 'tx'
          ? ['kg', 'wy', 'mg']  // 修改：酷狗优先于网易云
          : sourceKey === 'kg'
            ? ['tx', 'wy', 'mg']  // 修改：QQ音乐优先于网易云
            : sourceKey === 'mg'
              ? ['tx', 'kg', 'wy']  // 修改：QQ音乐和酷狗优先于网易云
              : ['tx', 'kg', 'wy', 'mg'];
      let candidate = null;
      let candidateSource = '';
      console.log('[LXSearchAndUrl] 开始跨平台搜索，顺序:', probeSourceOrder);
      for (const probe of probeSourceOrder) {
        try {
          console.log('[LXSearchAndUrl] 尝试平台:', probe);
          const match = await searchCrossPlatformCandidate(probe, songInputWithQuery);
          console.log('[LXSearchAndUrl] 平台', probe, '搜索结果:', match ? '找到匹配' : '未找到');
          if (match) {
            candidate = match;
            candidateSource = probe;
            break;
          }
        } catch (e) {
          console.warn('[LXFallback] probe', probe, 'failed:', e && (e.message || e));
        }
      }
      console.log('[LXSearchAndUrl] 跨平台搜索完成，candidate:', candidate ? '找到' : '未找到');
      // 2) 兜底：原平台本身再试一次
      if (!candidate) {
        console.log('[LXSearchAndUrl] 在原平台', sourceKey, '再试一次');
        try {
          const samePlatform = await searchCrossPlatformCandidate(sourceKey, songInputWithQuery);
          if (samePlatform) { candidate = samePlatform; candidateSource = sourceKey; }
        } catch (e) { /* ignore */ }
      }
      if (!candidate) {
        console.log('[LXSearchAndUrl] 所有平台都未找到匹配');
        sendJSON(res, { provider: 'lx', url: '', playable: false, error: 'LX_SEARCH_NO_CANDIDATE' });
        return;
      }
      console.log('[LXSearchAndUrl] 找到候选:', { source: candidateSource, name: candidate.name, artist: candidate.artist });
      console.log('[LXSearchAndUrl] 开始构建 LX musicInfo');
      const lxMusicInfo = buildLxMusicInfoFromMatch(candidateSource, candidate);
      console.log('[LXSearchAndUrl] LX musicInfo:', lxMusicInfo);
      if (!lxMusicInfo || !lxMusicInfo.meta || !lxMusicInfo.meta.songId) {
        console.log('[LXSearchAndUrl] musicInfo 缺少 songId');
        sendJSON(res, { provider: 'lx', url: '', playable: false, error: 'LX_CANDIDATE_MISSING_ID' });
        return;
      }
      // 3) 用 LX 脚本解析 URL
      console.log('[LXSearchAndUrl] 准备调用 lxSourceManager.resolveSongUrl');
      const resolvePayload = {
        lxSourceScriptId: '',
        lxSourceName: '',
        lxOriginSource: candidateSource,
        lxMusicInfo,
        lxQualitys: [],
        lxCandidates: [],
        quality,
      };
      const result = await lxSourceManager.resolveSongUrl(resolvePayload);
      console.log('[LXSearchAndUrl] LX 返回结果:', { hasUrl: !!result.url, error: result.error });
      if (!result || !result.url) {
        console.log('[LXSearchAndUrl] LX 未返回可播放URL');
        sendJSON(res, { provider: 'lx', url: '', playable: false, error: 'LX_RESOLVE_EMPTY', candidate: { source: candidateSource, name: candidate.name, artist: candidate.artist } });
        return;
      }
      console.log('[LXSearchAndUrl] ✓ 成功，准备返回结果');
      const proxyContextToken = rememberLxAudioProxyContext(result.headers);
      sendJSON(res, Object.assign({}, result, {
        playable: !!result.url,
        proxyContextToken,
        matchedSong: { source: candidateSource, name: candidate.name, artist: candidate.artist, musicInfo: lxMusicInfo },
      }));
    } catch (err) {
      console.error('[LXSearchAndUrl]', err);
      sendJSON(res, { provider: 'lx', url: '', playable: false, error: err.code || err.message || 'LX_SEARCH_AND_URL_FAILED' }, 500);
    }
    return;
  }
  ...
  if (pn === '/api/qishui/song/url') {
    try {
      ...
      // 修复：如果汽水音乐无法播放且启用了LX音源，强制触发LX接管
      if (!info.url && info.playable === false) {
        const lxSources = lxSourceManager && lxSourceManager.listSources ? lxSourceManager.listSources() : [];
        const hasReadyLxSource = lxSources.some(s => s.enabled && s.initStatus === 'ready');

        if (hasReadyLxSource) {
          console.log('[QishuiSongUrl] 无法播放，已启用LX音源，返回 trial_only 触发LX接管');
          const modifiedInfo = Object.assign({}, info, {
            reason: 'trial_only',
            category: 'trial_only',
            restriction: Object.assign({}, info.restriction || {}, { category: 'trial_only', reason: 'trial_only' }),
            message: '汽水音乐无法播放，尝试使用LX音源',
          });
          sendJSON(res, modifiedInfo);
          return;
        }
      }
  ...
  if (pn === '/api/kugou/song/url') {
    try {
      ...
      // 修复：如果酷狗无法播放且启用了LX音源，强制触发LX接管
      if (!info.url && info.playable === false) {
        const lxSources = lxSourceManager && lxSourceManager.listSources ? lxSourceManager.listSources() : [];
        const hasReadyLxSource = lxSources.some(s => s.enabled && s.initStatus === 'ready');

        if (hasReadyLxSource) {
          console.log('[KugouSongUrl] 无法播放，已启用LX音源，返回 trial_only 触发LX接管');
          const modifiedInfo = Object.assign({}, info, {
            reason: 'trial_only',
            category: 'trial_only',
            restriction: Object.assign({}, info.restriction || {}, { category: 'trial_only', reason: 'trial_only' }),
            message: '酷狗无法播放，尝试使用LX音源',
          });
          sendJSON(res, modifiedInfo);
          return;
        }
  ...
  if (pn === '/api/audio') {
    try {
      ...
      const proxyContextToken = url.searchParams.get('ctx') || url.searchParams.get('context') || '';
      const extraHeaders = lxAudioProxyContext(proxyContextToken) || null;
      ...
      const hdr = audioProxyHeadersFor(audioUrl, range, extraHeaders);
