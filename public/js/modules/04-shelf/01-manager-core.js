function makeShelfManager() {
  ...
  function currentItems() {
    if (hasAnyPlatformLogin() && (userPlaylists.length || myPodcastCollections.length)) {
      ...
      var items = source.map(function (pl) {
        var provider = pl.provider === 'local' ? 'local' : (pl.provider === 'qq' ? 'qq' : (pl.provider === 'kugou' ? 'kugou' : (pl.provider === 'qishui' ? 'qishui' : (pl.provider === 'spotify' ? 'spotify' : 'netease'))));
        var sourceLabel = provider === 'local' ? '本地' : (provider === 'qq' ? 'QQ' : (provider === 'kugou' ? 'KG' : (provider === 'qishui' ? 'QS' : (provider === 'spotify' ? 'SP' : 'NE'))));
        ...
        return {
          ...
          cover: pl.cover || '', tag: provider === 'local' ? '本地歌单' : ((pl.shelfPane || pl.shelf_pane) === 'fav' || (!(pl.shelfPane || pl.shelf_pane) && pl.subscribed) ? '收藏歌单' : (provider === 'qishui' ? '汽水歌单' : '我的歌单')), playlistId: (provider === 'local' ? 'local:' : (provider === 'qq' ? 'qq:' : (provider === 'kugou' ? 'kugou:' : (provider === 'qishui' ? 'qishui:' : '')))) + pl.id, provider: provider
        };
      });
  ...
  function playPlaylistCard(card) {
    ...
    if (String(action.playlistId).indexOf('local:') === 0 && typeof loadLocalPlaylistIntoQueue === 'function') loadLocalPlaylistIntoQueue(true, 0);
    else loadPlaylistIntoQueueById(action.playlistId, true, action.title || (card.item && card.item.title) || '');
    return true;
  }
