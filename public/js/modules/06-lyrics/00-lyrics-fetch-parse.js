...
function persistentLyricCacheKey(song) {
  ...
  if (provider === 'lx') {
    ...
    var lxMusicInfo = song.lxMusicInfo || {};
    var lxMeta = lxMusicInfo.meta || {};
    var originId = lxMeta.songId || lxMeta.hash || lxMeta.strMediaMid || '';
    var artist = song.artist || song.singer || song.artists || '';
    return ['lyrics-v1', 'lx-' + lxOriginSource, originId, song.name || song.title || '', artist].join('|');
  }
