...
function playlistPanelDetailRowsHtml(options) {
  ...
  var isLocalPlaylist = String(st.key || '').indexOf('local:') === 0;
  ...
  rows += tracks.slice(start, end).map(function (song, localIndex) {
    ...
    return '<div class="pl-detail-row" data-pl-detail-row="' + i + '">' +
      ...
      (isLocalPlaylist ? '<button type="button" class="pl-detail-remove" data-local-pl-remove="' + i + '" title="从本地歌单移除">×</button>' : '') +
      '</div>';
  }).join('');
...
async function loadMorePlaylistPanelDetailTracks(reason) {
  ...
  if (provider === 'local') {
    var localTracks = (localPlaylistSongs || []).map(cloneSong);
    ...
    st.error = '';
    st.message = localTracks.length ? '' : '本地歌单还是空的，可以从搜索结果添加';
    renderPlaylistPanelDetailState();
    if (reason === 'initial') scrollPlaylistPanelDetailIntoView(st.key);
...
function playPlaylistPanelDetailTrack(index) {
  ...
  var pid = parts.slice(1).join(':');
  if (provider === 'local') {
    loadLocalPlaylistIntoQueue(true, index);
    return;
  }
...
function renderUserPlaylistsList(opts) {
  ...
  if (typeof ensureLocalPlaylistCatalogPresence === 'function') ensureLocalPlaylistCatalogPresence();
...
document.getElementById('pl-list').addEventListener('click', function (e) {
  ...
  var localRemove = e.target && e.target.closest ? e.target.closest('[data-local-pl-remove]') : null;
  if (localRemove) {
    e.preventDefault();
    e.stopPropagation();
    removeLocalPlaylistSong(Number(localRemove.getAttribute('data-local-pl-remove')));
    return;
  }
