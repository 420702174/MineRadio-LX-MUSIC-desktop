var lxUrlCache = {}; // LX URL 缓存: { songKey: { url, cachedAt, quality } }
var LX_URL_CACHE_TTL_MS = 10 * 60 * 1000; // 缓存10分钟

function getLxUrlCacheKey(song) {
  if (!song) return '';
  return [
    song.name || '',
    song.artist || '',
    song.lxSourceScriptId || '',
    song.lxOriginSource || ''
  ].join('|');
}

function getLxCachedUrl(song, requestedQuality) {
  var key = getLxUrlCacheKey(song);
  if (!key) return null;
  var cached = lxUrlCache[key];
  if (!cached || !cached.url) return null;
  // 检查是否过期
  if (Date.now() - cached.cachedAt > LX_URL_CACHE_TTL_MS) {
    delete lxUrlCache[key];
    return null;
  }
  // 如果音质匹配，返回缓存
  if (cached.quality === requestedQuality) {
    return cached;
  }
  return null;
}

function setLxUrlCache(song, data, requestedQuality) {
  var key = getLxUrlCacheKey(song);
  if (!key || !data || !data.url) return;
  lxUrlCache[key] = {
    url: data.url,
...
async function resolveAlbumGaplessPlaybackData(song) {
  ...
  if (playbackProvider === 'lx') {
    return apiJson('/api/lx/song/url', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        song: {
          provider: 'lx',
          lxSourceScriptId: song.lxSourceScriptId || '',
          lxSourceName: song.lxSourceName || '',
          lxOriginSource: song.lxOriginSource || '',
          lxMusicInfo: song.lxMusicInfo || null,
          lxQualitys: song.lxQualitys || [],
          lxCandidates: song.lxCandidates || []
        },
...
async function scheduleAlbumGaplessPreloadForCurrent(token, reason) {
  ...
  try {
    ...
    var proxyAudioUrl = '/api/audio?url=' + encodeURIComponent(data.url) + (data.proxyContextToken ? ('&ctx=' + encodeURIComponent(data.proxyContextToken)) : '');
    var media = new Audio();
...
async function playQueueAt(idx, opts) {
  ...
  try {
    ...
    try {
      ...
      var isLxPlayback = playbackProvider === 'lx';
      ...
      if (albumGaplessHandoff) {
        ...
        data = await apiJson('/api/spotify/song/url?id=' + encodeURIComponent(song.id || song.providerSongId || song.spotifyId || '') +
          ...
          '&uri=' + encodeURIComponent(song.spotifyUri || song.uri || '') +
          qualityParam, { timeoutMs: 9000 });
      } else if (isLxPlayback) {
        // 先尝试从缓存获取 URL
        var cachedData = getLxCachedUrl(song, requestedQuality);
        if (cachedData) {
          data = cachedData;
        } else {
          // 构建请求payload，确保所有字段都可序列化
          var lxPayload = {
            song: {
              provider: 'lx',
              lxSourceScriptId: song.lxSourceScriptId || '',
              lxSourceName: song.lxSourceName || '',
              lxOriginSource: song.lxOriginSource || '',
              // 修复：如果 lxMusicInfo 包含 musicInfo 属性，提取内层的 musicInfo
              lxMusicInfo: (song.lxMusicInfo && song.lxMusicInfo.musicInfo) ? song.lxMusicInfo.musicInfo : (song.lxMusicInfo || null),
              lxQualitys: Array.isArray(song.lxQualitys) ? song.lxQualitys : [],
              lxCandidates: Array.isArray(song.lxCandidates) ? song.lxCandidates : []
            },
            quality: requestedQuality
          };
          ...
          if (!data) {
            ...
            try {
              // 检查 token 是否在 await 前就变了
              if (token !== trackSwitchToken) {
                data = { provider: 'lx', url: '', playable: false, error: 'TOKEN_MISMATCH' };
              } else {
                var res = await fetch('/api/lx/song/url', {
                  method: 'POST',
                  headers: { 'Content-Type': 'application/json' },
                ...
                // 如果成功获取到 URL，缓存起来
                if (data && data.url) {
                  setLxUrlCache(song, data, requestedQuality);
                }
              }
            } catch (err) {
              data = { provider: 'lx', url: '', playable: false, error: err.message || 'LX_REQUEST_FAILED' };
            }
          }
        }
      } else {
      ...
      // 试听片段 (data.trial=true) 也属于受限场景，需要触发 LX 源接管或平台换源
      var dataIsUnavailable = !data || !data.url;
      var dataIsTrialOnly = !!(data && data.trial && data.url);
      if (dataIsUnavailable || dataIsTrialOnly) {
        var fallbackPayload = dataIsTrialOnly
          ? Object.assign({}, data, { url: null, reason: dataIsTrialOnly ? 'trial_only' : (data && data.reason) || 'url_unavailable' })
          : data;
        var fallbackResult = await tryAutoPlaybackFallback(song, fallbackPayload, idx, token, retryPlaybackOpts);
      ...
      var proxyAudioUrl = opts.preloadedProxyAudioUrl || '/api/audio?url=' + encodeURIComponent(data.url) + (data.proxyContextToken ? ('&ctx=' + encodeURIComponent(data.proxyContextToken)) : '');
      if (albumGaplessHandoff) {
        audioFadeSerial++;
        clearAudioFadeTimers();
