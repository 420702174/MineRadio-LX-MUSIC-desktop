...
function playbackQualityLabel(value, provider) {
  ...
  if (provider === 'lx') {
    if (value === 'hires' || value === 'jymaster') return '导入源高音质';
    if (value === 'lossless') return '导入源无损';
    if (value === 'exhigh') return '导入源高品质';
    return '导入源标准';
  }
...
function playbackQualityShortLabel(value, provider) {
  ...
  if (provider === 'lx') {
    if (value === 'hires' || value === 'jymaster') return 'LX HQ';
    if (value === 'lossless') return 'LX SQ';
    if (value === 'exhigh') return 'LX 320';
    return 'LX 128';
  }
...
function readPlaybackQualityPreference() {
  ...
  try {
    ...
    lx: normalizePlaybackQualityForProvider(parsed.lx || fallback.lx || 'hires', 'lx')
    };
  } catch (e) {
    return fallback;
  }
...
function updatePlaybackQualityUi() {
  ...
  var qualityProviderTitle = provider === 'spotify' ? 'Spotify 匹配源: ' : (provider === 'qishui' ? '汽水音质: ' : (provider === 'lx' ? 'LX 导入源音质: ' : (provider === 'qq' ? 'QQ 音质: ' : (provider === 'kugou' ? '酷狗音质: ' : '网易云音质: '))));
...
function canReloadCurrentTrackForQuality() {
  ...
  return songProviderKey(song) === 'netease' || songProviderKey(song) === 'qq' || songProviderKey(song) === 'kugou' || songProviderKey(song) === 'lx';
}
