...
function songSourceLabel(song) {
  if (!song) return '未知';
  if (song.provider === 'lx' || song.source === 'lx' || song.lxSourceScriptId) return lxSourceSubLabel(song);
...
function openCollectModal(song) {
  var provider = songAccountProvider(song);
  var adapter = songAccountAdapter(provider);
  var canUseLocalPlaylist = typeof addSongToLocalPlaylist === 'function';
  if (!canUseLocalPlaylist && (!adapter || !adapter.collect || !adapter.playlistAddUrl)) {
  ...
  if (adapter && adapter.collect && isSongAccountLoggedIn(provider)) {
...
function renderCollectModal() {
  ...
  var adapter = songAccountAdapter(provider);
  var rows = [];
  if (typeof localPlaylistCatalogItem === 'function') {
    var local = localPlaylistCatalogItem();
    rows.push('<div class="collect-item local-collect-item" data-collect-pid="' + escHtml(String(local.id || '')) + '" onclick="addCollectTargetToPlaylist(this.getAttribute(\'data-collect-pid\'))">' +
      (local.cover ? '<img src="' + escHtml(local.cover) + '" alt="">' : '<div class="cover-placeholder"></div>') +
      '<div style="min-width:0"><div class="collect-title">' + escHtml(local.name || '本地歌单') + '</div><div class="collect-sub">' + (local.trackCount || 0) + ' 首 · 无需登录</div></div>' +
      '</div>');
  }
  if (!adapter || !adapter.collect) {
    list.innerHTML = rows.join('') || '<div class="collect-empty">' + escHtml(songAccountUnsupportedMessage(provider, 'collect')) + '</div>';
    return;
  }
  if (!isSongAccountLoggedIn(provider)) {
    rows.push('<div class="collect-empty">登录' + escHtml(adapter.label) + '后可同步收藏到平台歌单</div>');
    list.innerHTML = rows.join('');
    return;
  }
  if (!userPlaylists.length) {
    list.innerHTML = rows.join('') + miniQueueSkeleton();
    return;
  }
  ...
  if (!mine.length) {
    rows.push('<div class="collect-empty">还没有可写入的平台歌单，可以先新建一个</div>');
    list.innerHTML = rows.join('');
    return;
  }
  list.innerHTML = rows.join('') + mine.map(function (pl) {
...
async function createPlaylistFromCollect() {
  ...
  if (!adapter || !adapter.createPlaylist || !adapter.playlistCreateUrl) {
    if (collectTargetSong && typeof addSongToLocalPlaylist === 'function') {
      addSongToLocalPlaylist(collectTargetSong);
      closeCollectModal();
      return;
    }
...
async function addCollectTargetToPlaylist(pid) {
  ...
  if (String(pid) === 'local-default' && typeof addSongToLocalPlaylist === 'function') {
    addSongToLocalPlaylist(targetSong);
    closeCollectModal();
    return;
  }
