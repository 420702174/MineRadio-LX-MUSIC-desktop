function queueSong(song, opts) {
  opts = opts || {};
  if (!song) return -1;
  if (typeof markQueueAsManualPlaybackSource === 'function') markQueueAsManualPlaybackSource('queue-song');
  ...
  if (typeof renderLocalPlaylistSwitcher === 'function') renderLocalPlaylistSwitcher();
  return insertAt;
}
...
function playSearchResult(i) {
  var song = playlist[i]; if (!song) return;
  if (typeof markQueueAsManualPlaybackSource === 'function') markQueueAsManualPlaybackSource('play-search-result');
