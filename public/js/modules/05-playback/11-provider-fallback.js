...
// 触发 LX 源优先接管的业务平台集合（网易云 / QQ / 酷狗 / 汽水 / Spotify）
var LX_FALLBACK_ELIGIBLE_PROVIDERS = ['netease', 'qq', 'kugou', 'qishui', 'spotify'];

function categoryLabel(category) {
  if (category === 'vip_required') return 'VIP 曲目';
  if (category === 'paid_required') return '付费/数字专辑曲目';
  if (category === 'trial_only') return '试听片段';
  if (category === 'login_required') return '未登录试听';
  return '不可播';
}

function playbackProviderLabel(song) {
  var provider = songProviderKey(song);
  if (provider === 'lx') return lxSourceDisplayName(song);
...
function playbackRestrictionCategory(song, data) {
  ...
  // 修复：如果 category 已经是 trial_only，保持不变，让LX接管处理
  if (category === 'trial_only') return category;
...
async function tryAutoPlaybackFallback(song, data, idx, token, opts) {
  opts = opts || {};
  console.log('[tryAutoPlaybackFallback] ========== 被调用 ==========');
  console.log('[tryAutoPlaybackFallback] 歌曲:', song && song.name, '-', song && song.artist);
  console.log('[tryAutoPlaybackFallback] data:', {
    url: data && data.url ? '有URL' : '无URL',
  ...
  if (songProviderKey(song) === 'lx') {
    // LX音源播放失败，尝试回退到原始音源
    console.log('[tryAutoPlaybackFallback] LX音源失败，检查是否可以回退到原始音源');

    if (song.originalProvider && song.originalProvider !== 'lx') {
      console.log('[tryAutoPlaybackFallback] LX音源失败，回退到原始音源:', song.originalProvider);

      // 恢复原始的provider和source
      var restoredSong = Object.assign({}, song, {
        provider: song.originalProvider,
        source: song.originalSource || song.originalProvider,
      });

      // 更新队列中的歌曲
      playQueue[idx] = hydrateCustomCover(restoredSong);
      safeRenderQueuePanel('lx-fallback-restore', { scrollCurrent: miniQueueOpen });
      safeShelfRebuild('lx-fallback-restore');

      // 使用原始音源继续fallback流程
      song = restoredSong;
      console.log('[tryAutoPlaybackFallback] 已恢复到原始音源，继续fallback流程');
    } else {
      console.log('[tryAutoPlaybackFallback] LX音源失败且没有原始音源信息，跳过此曲目');
      return await skipFailedQueueItem(idx, token, '导入音源没有返回可播放地址，正在尝试队列里的下一首。', { playbackOpts: opts });
    }
  }
  var category = playbackRestrictionCategory(song, data);
  console.log('[tryAutoPlaybackFallback] 计算出的 category:', category);
  ...
  // ★ 优先 LX 源接管：VIP / 付费 / 试听 / 登录受限 场景
  console.log('[LXFallback] 检查是否触发LX接管:', {
    lxAttempted: recovery.lxAttempted,
    category: category,
    categoryMatch: category === 'vip_required' || category === 'paid_required' || category === 'trial_only' || category === 'login_required',
    provider: normalizePlaybackProvider(songProviderKey(song)),
    eligibleProvider: LX_FALLBACK_ELIGIBLE_PROVIDERS.indexOf(normalizePlaybackProvider(songProviderKey(song))) >= 0,
    lxAvailable: typeof lxSourcesAvailable === 'function' ? lxSourcesAvailable() : 'function不存在',
    searchFnExists: typeof searchLxCandidateForSong === 'function'
  });
  if (
    !recovery.lxAttempted
    && (category === 'vip_required' || category === 'paid_required' || category === 'trial_only' || category === 'login_required')
    && LX_FALLBACK_ELIGIBLE_PROVIDERS.indexOf(normalizePlaybackProvider(songProviderKey(song))) >= 0
    && typeof lxSourcesAvailable === 'function'
    && lxSourcesAvailable()
    && typeof searchLxCandidateForSong === 'function'
  ) {
    console.log('[LXFallback] ✓ 所有条件满足，开始LX接管');
    recovery.lxAttempted = true;
    var lxCandidate = null;
    try {
      console.log('[LXFallback] 调用 searchLxCandidateForSong...');
      console.log('[LXFallback] recovery剩余时间:', sourceFallbackRecoveryRemainingMs(recovery), 'ms');
      lxCandidate = await awaitSourceFallbackBudget(
        searchLxCandidateForSong(song, { skipCache: !!opts.lxSkipCache }),
        recovery
      );
      console.log('[LXFallback] searchLxCandidateForSong 返回:', lxCandidate ? '有结果' : '无结果', lxCandidate);
    } catch (e) {
      console.warn('[LXFallback] searchLxCandidateForSong 异常:', e && (e.message || e));
      lxCandidate = null;
    }
    if (token !== trackSwitchToken) return false;
    if (lxCandidate !== sourceFallbackBudgetTimeoutResult && lxCandidate && lxCandidate.url) {
      var lxPlaybackData = {
        url: lxCandidate.url,
        level: lxCandidate.level || lxCandidate.quality || '',
        quality: lxCandidate.quality || lxCandidate.level || '',
        source: 'lx',
        provider: 'lx',
        headers: lxCandidate.headers || {},
        proxyContextToken: lxCandidate.proxyContextToken || '',
        lxSourceScriptId: lxCandidate.lxSourceScriptId || '',
        lxSourceName: lxCandidate.lxSourceName || '',
        lxOriginSource: lxCandidate.lxOriginSource || '',
        autoFallbackFrom: songProviderKey(song),
        reason: 'lx_fallback_resolved',
        vipRequired: false,
        trial: false,
        needVip: false,
        onlyVipPlayable: false,
        loggedIn: true,
        restriction: { category: 'lx_fallback_resolved', reason: 'lx_fallback_resolved', message: '' },
      };
      var lxOriginalSong = playQueue[idx];
      var originalProvider = songProviderKey(song);
      console.log('[LX接管] 开始构造lxCommittedSong, originalProvider:', originalProvider);
      var lxCommittedSong = hydrateCustomCover(Object.assign({}, song, {
        provider: 'lx',
        source: 'lx',
        lxSourceScriptId: lxCandidate.lxSourceScriptId || song.lxSourceScriptId || '',
        lxSourceName: lxCandidate.lxSourceName || song.lxSourceName || '',
        lxOriginSource: lxCandidate.lxOriginSource || (lxCandidate.matchedSong && lxCandidate.matchedSong.source) || song.lxOriginSource || '',
        lxMusicInfo: (lxCandidate.matchedSong && lxCandidate.matchedSong.musicInfo) || song.lxMusicInfo || null,  // 修复：从 matchedSong.musicInfo 获取
        lxQualitys: lxCandidate.lxQualitys || song.lxQualitys || [],      // 保存可用音质列表
        lxCandidates: lxCandidate.lxCandidates || song.lxCandidates || [], // 保存候选列表
        originalProvider: originalProvider,  // 保存原始provider，以便LX失败时回退
        originalSource: originalProvider,    // 保存原始source
        vipRequired: false,
        trial: false,
        preview: false,
        needVip: false,
        ...
        isVip: false,
        isVIP: false,
        _lxFallbackFrom: originalProvider,
      }));
      console.log('[LX接管] lxCommittedSong构造完成:', {
        name: lxCommittedSong.name,
        provider: lxCommittedSong.provider,
        originalProvider: lxCommittedSong.originalProvider,
        originalSource: lxCommittedSong.originalSource,
        hasLxMusicInfo: !!(lxCommittedSong.lxMusicInfo)
      });
      playQueue[idx] = lxCommittedSong;
      console.log('[LX接管] 已更新playQueue[' + idx + '], 验证:', {
        provider: playQueue[idx].provider,
        originalProvider: playQueue[idx].originalProvider,
        originalSource: playQueue[idx].originalSource
      });
      safeRenderQueuePanel('lx-fallback-provisional', { scrollCurrent: miniQueueOpen });
      safeShelfRebuild('lx-fallback-provisional');
      var lxPlaybackOpts = {
        fallbackDepth: 1,
        startupAutoplay: !!opts.startupAutoplay,
        preserveHomeState: !!opts.preserveHomeState,
        suppressPlayFailureNotice: true,
        preResolvedPlaybackData: lxPlaybackData,
        fallbackOriginalSong: lxOriginalSong,
        fallbackCandidateSong: lxCommittedSong,
        sourceFallbackRecovery: recovery,
        qqQualityTried: ['hires', 'lossless', 'exhigh', 'standard'],
      };
      if (opts.resumeAt != null) lxPlaybackOpts.resumeAt = opts.resumeAt;
      var lxFallbackToken = trackSwitchToken;
      var lxStarted = await playQueueAt(idx, lxPlaybackOpts);
      if (lxFallbackToken !== trackSwitchToken) return false;
      if (lxStarted === true) {
        completeSourceFallbackRecovery(recovery);
        if (!opts.startupAutoplay) {
          var lxLabel = lxCandidate.lxSourceName || 'LX 源';
          showSourceFallbackNotice(
            'LX 源已接管播放',
            fromLabel + ' 当前为 ' + categoryLabel(category) + '，已使用 ' + lxLabel + ' 播放。'
          );
          showToast('LX 源已接管: ' + lxLabel);
        }
        return true;
      }
      // LX 解析成功但音频装载失败：回滚队列项后继续走平台换源
      restoreSourceFallbackQueueItem(idx, lxOriginalSong, lxCommittedSong, lxFallbackToken);
      token = lxFallbackToken;
      recovery.lxAttempted = true;
      recovery.lxFailed = true;
    } else {
      recovery.lxAttempted = true;
      recovery.lxFailed = true;
    }
  ...
  // LX 已尝试但失败 + 没有任何其它已登录平台 + 需要登录 → 退回原逻辑让上层打开登录入口
  if (
    recovery.lxFailed
    && !alternateProviders.length
    && category === 'login_required'
    && opts.fallbackDepth === 0
  ) {
    return null;
  }
