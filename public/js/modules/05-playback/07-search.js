...
function songProviderKey(song) {
  if (song && (song.provider === 'lx' || song.source === 'lx' || song.lxSourceScriptId)) return 'lx';
...
function songSourceTagHtml(song, opts) {
  ...
  var label = key === 'qq' ? 'QQ' : (key === 'kugou' ? 'KG' : (key === 'qishui' ? 'QS' : (key === 'spotify' ? 'SP' : (key === 'lx' ? 'LX' : 'NE'))));
  if (opts.switcher) {
    if (key === 'lx') return '<span class="tag-source ' + key + ' control-source-chip" title="' + escHtml(lxSourceSubLabel(song)) + '">' + label + '</span>';
...
function searchResultMetaText(song) {
  ...
  if (songProviderKey(song) === 'lx') bits.push(lxSourceSubLabel(song));
...
function mergeSongSearchResults(neteaseSongs, qqSongs, kugouSongs, qishuiSongs, spotifySongs, limit, q) {
  var lxSongs = Array.isArray(neteaseSongs) && neteaseSongs.__lxSongs ? neteaseSongs.__lxSongs : [];
  if (lxSongs.length) neteaseSongs = Array.prototype.slice.call(neteaseSongs || []);
  var out = [];
  var providerSeen = {};
  var canonicalSeen = {};
  function push(song, sourceIndex) {
    ...
    if (canonicalKey && canonicalSeen[canonicalKey] != null) {
      ...
      var existingSong = out[existingIndex];
      var existingIsLx = songProviderKey(existingSong) === 'lx';
      var nextIsLx = songProviderKey(song) === 'lx';
      if (existingIsLx !== nextIsLx) {
        if (nextIsLx) out[existingIndex] = song;
      } else if ((song._searchScore || 0) > (existingSong._searchScore || 0)) out[existingIndex] = song;
      return;
    }
  ...
  out.sort(function (a, b) {
    var aIsLx = songProviderKey(a) === 'lx';
    var bIsLx = songProviderKey(b) === 'lx';
    if (aIsLx !== bIsLx) return aIsLx ? -1 : 1;
...
async function fetchMusicSearchResults(q, mode, previousPages) {
  ...
  var pageLimitByProvider = { netease: 18, qq: 12, kugou: 12, qishui: 12, spotify: 10, lx: 18 };
  var lxSongs = [];
  if (mode === 'song' && (!previousPages || previousPages.lx == null || previousPages.lx.hasMore)) {
    try {
      var lxPrevious = previousPages && previousPages.lx;
      var lxOffset = lxPrevious ? Math.max(0, Number(lxPrevious.nextOffset) || 0) : 0;
      var lxLimit = pageLimitByProvider.lx;
      var lxResponse = await apiJson('/api/lx/search?keywords=' + encodeURIComponent(q) + '&limit=' + lxLimit + '&offset=' + lxOffset, { timeoutMs: 8000 });
      lxSongs = Array.isArray(lxResponse && lxResponse.songs) ? lxResponse.songs : [];
      providerPages.lx = {
        offset: lxOffset,
        limit: lxLimit,
        nextOffset: Number(lxResponse && lxResponse.nextOffset) || (lxOffset + lxSongs.length),
        hasMore: !!(lxResponse && lxResponse.hasMore),
        total: Number(lxResponse && lxResponse.total) || lxSongs.length,
        failed: false
      };
    } catch (error) {
      console.warn('LX search failed:', error);
      providerPages.lx = Object.assign({}, providerPages.lx || {}, { hasMore: false, failed: true });
    }
  }
  ...
  songsByProvider.netease = songsByProvider.netease || [];
  songsByProvider.netease.__lxSongs = lxSongs;
