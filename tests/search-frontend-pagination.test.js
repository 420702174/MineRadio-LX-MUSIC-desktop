...
test('LX search results stay ahead of built-in platforms in All mode merges', () => {
  const sandbox = {
  ...
  vm.runInNewContext(functionBundle([
    ...
    this.merge = mergeSongSearchResults;
  `), sandbox);

  const neteaseSongs = [{ provider: 'netease', id: 'ne-1', name: '稻香', artist: '周杰伦', popularity: 999 }];
  neteaseSongs.__lxSongs = [{ provider: 'lx', id: 'lx-1', name: '晴天', artist: '周杰伦', popularity: 1 }];
  const merged = sandbox.merge(neteaseSongs, [], [], [], [], 10, '晴天');
  assert.equal(merged[0].provider, 'lx');
  assert.equal(merged[1].provider, 'netease');
});
