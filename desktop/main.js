...
function bindStartupFailureHandlers() {
  ...
  process.on('unhandledRejection', (reason) => {
    const _lxErrorText = String(reason && reason.stack || reason && reason.message || reason || '');
    if (/LX_HTTP_TIMEOUT|LX_HTTP_FAILED|LX http request timed out|fetch failed|All promises were rejected/i.test(_lxErrorText)) {
      console.warn('[UnhandledRejection] LX source network error (non-fatal):', reason.message || reason);
      return;
    }
    if (startupCompleted) {
...
ipcMain.handle('mineradio-lx-source-choose-file', async (event) => {
  try {
    if (!isTrustedMainWindowIpc(event)) return { ok: false, error: 'UNTRUSTED_SENDER' };
    const owner = getSenderWindow(event);
    const result = await dialog.showOpenDialog(owner, {
      title: '导入 LX 自定义音源脚本（可多选）',
      properties: ['openFile', 'multiSelections'],
      filters: [
        { name: 'LX 音源脚本', extensions: ['js', 'lx', 'txt'] },
    ...
    return { ok: false, error: e.message || 'LX_SOURCE_IMPORT_FAILED' };
  }
});
...
function configureLocalServerEnvironment(port) {
  ...
  process.env.MINERADIO_LX_SOURCE_FILE = path.join(STABLE_USER_DATA_PATH, 'lx-sources.json');
